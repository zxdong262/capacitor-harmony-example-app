...
//
// NOTE: a fresh `http.Server` is created per attempt — re-listening the same
// server after an `error` + async `close()` is unreliable
// (`ERR_SERVER_ALREADY_LISTEN`) and can leave the backend dead.
...
function listen(index) {
  ...
  server.once('error', (err) => {
    ...
    // Next attempt uses a brand-new server so no stale state leaks across.
